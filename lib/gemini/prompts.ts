...
/**
 * 노트 태그 생성을 위한 프롬프트 생성
 * @param noteContent - 태그를 생성할 노트 내용
 * @returns 태그 생성을 위한 프롬프트 문자열
 */
export function createTagPrompt(noteContent: string): string {
  return `다음 노트 내용을 분석하여 관련성 높은 태그를 최대 6개까지 생성해주세요.

태그 생성 규칙:
- 최대 6개까지의 관련성 높은 키워드 생성
- 각 태그는 1-3단어로 간결하게 작성
- 노트의 핵심 주제, 내용, 분야를 반영
- 한국어 또는 영어로 작성 (일관성 있게)
- 태그는 쉼표로 구분하여 나열
- 불필요한 관사나 조사 제외
- 구체적이고 명확한 의미를 가진 단어 사용
